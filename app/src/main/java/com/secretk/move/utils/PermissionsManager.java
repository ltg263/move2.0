        ...
        if (Build.VERSION.SDK_INT >= 23) {
            if (addPermission(mActivity,permission)) {
                if (!mActivity.shouldShowRequestPermissionRationale(permission)) {
                    ...
                    //已禁止过，弹窗提示
                    showPermission(mActivity,app.getString(R.string.permission_ask_now, permission), permission);
                }
            } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CODE_ASK_PERMISSIONS) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                ToastUtils.getInstance().show(app, app.getString(R.string.permission_ask_error, (permissions[0])));
                if (mCheckCallBack != null) {
